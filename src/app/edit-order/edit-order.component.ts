...
export class EditOrderComponent {
  ...
  updateTotalAmount() {
    this.totalAmount = this.subTotal + this.orderData[0].tax;
    this.totalAmount = Number(this.totalAmount.toFixed(2));
  }

  ngOnInit(): void {

    this.http.postApi("http://localhost:8080/order/get__order", this.dataService.editID).subscribe((res: any) => {
      ...
      if (result == "sure") {
        ...
        for (let item of this.infos) {
          infoData.push({
            ...
            "orderID": this.orderData[0].orderID,
        ...
        let req = this.orderData[0];

        this.http.postApi("http://localhost:8080/order/edit_order", req)
          .subscribe({
